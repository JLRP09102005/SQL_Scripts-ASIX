...
-- PENALTIES
-- =================================================================
CREATE PROCEDURE sp_InsertPenaltyWithResult (
...
BEGIN
    ...
    -- Si aplica a piloto, el id_pilot es obligatorio cuando hay resultado
    IF p_penalty_applies_to = 'PILOT' AND p_id_result IS NOT NULL AND p_id_pilot IS NULL THEN
        SET v_error_message = 'id_pilot is required when penalty applies to PILOT and result is set';
        SIGNAL SQLSTATE '45004';
    END IF;
...
CREATE PROCEDURE sp_UpdatePenaltyWithResult (
    IN p_id_penalty INT,
    IN p_penalty_type CHAR(30),
    IN p_reason VARCHAR(100),
    IN p_penalty_value DECIMAL(7,2),
...
BEGIN
    ...
    -- Actualiza o elimina la relación con resultado
    DELETE FROM penalties_results WHERE id_penalty = p_id_penalty;

    IF p_id_result IS NOT NULL THEN
        INSERT INTO penalties_results (id_penalty, id_result)
        VALUES (p_id_penalty, p_id_result);
    END IF;
